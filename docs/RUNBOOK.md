...
## Плановое ТО
Плановое ТО — это список профилактических работ по конкретным лифтам. Оно отличается от аварийных/реактивных заявок тем, что само по себе не создаёт заявку мастеру и не запускает offline/PWA-сценарий. Диспетчер или админ ведёт план, отмечает факт выполнения и при необходимости отдельно создаёт обычную заявку.
...
- Нет cron-задач, календаря, автоуведомлений и автогенерации заявок.
- Нет отдельного technician/PWA-потока для планов ТО.
- Если по плану нужна работа мастера в мобильном приложении, диспетчер создаёт обычную заявку и назначает её существующим способом.
