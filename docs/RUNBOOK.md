...
- Для новой пустой базы создаётся только базовый штат: `admin`, `dispatcher` и пять активных мастеров с доступами `master1` ... `master5`. Тестовые лифты, заявки, клиенты, договоры и планы ТО автоматически не создаются.
...
## Плановое ТО
Плановое ТО — это список профилактических работ по конкретным лифтам. Оно отличается от аварийных/реактивных заявок тем, что само по себе не запускает offline/PWA-сценарий и не создаётся по расписанию. Диспетчер или админ ведёт план, проверяет очередь сроков, при необходимости создаёт обычную заявку и отмечает факт выполнения.

### Создать план ТО
1. Войти как admin или dispatcher.
2. Открыть вкладку «ТО».
3. Выбрать лифт из SQL-реестра. Если лифта нет, сначала создать его во вкладке «Лифты».
4. Заполнить название, интервал (`monthly`, `quarterly`, `semiannual`, `annual` или `custom`), следующую дату и при необходимости назначенного мастера.
5. Нажать «Сохранить». API: `POST /api/maintenance-plans`.

### Изменить или поставить на паузу
- Во вкладке «ТО» нажать «Редактировать», изменить поля и сохранить.
- Статусы плана: `active`, `paused`, `completed`, `overdue`.
- API: `PATCH /api/maintenance-plans/<id>`.

### Проверить очередь ТО
1. Войти как admin или dispatcher.
2. Открыть вкладку «ТО».
3. Сверху показаны счётчики:
   - просрочено: активные планы, где `next_due_date` раньше сегодняшней даты;
   - сегодня: активные планы на сегодняшнюю дату;
   - 7 дней: активные планы со сроком в ближайшие 7 дней;
   - 30 дней: активные планы со сроком в ближайшие 30 дней.
4. Таблица сгруппирована по этим же срокам и показывает лифт/объект, клиента/договор при наличии, название ТО, дату, статус и назначенного мастера.
5. Фильтры позволяют выбрать статус, мастера, диапазон дат или режим `overdue only`.

API очереди: `GET /api/maintenance-plans/due`. По умолчанию возвращаются активные планы до ближайших 30 дней и просроченные планы. `paused` и `completed` не смешиваются с активной очередью; чтобы увидеть их отдельно, используйте фильтр статуса или `include_inactive=1`.

Просрочка считается динамически: если `status == active` и `next_due_date < today`, план показывается как `overdue`, но сохранённый статус в базе не меняется автоматически.

### Создать заявку из плана ТО
1. В очереди ТО нажать «Создать заявку» у нужного активного плана.
2. API: `POST /api/maintenance-plans/<id>/generate-ticket`.
3. Создаётся обычная заявка с привязкой к тому же лифту, адресом/координатами лифта, приоритетом `MEDIUM` и описанием, где указаны название и дата планового ТО.
4. Если в плане указан мастер, заявка назначается ему; иначе используется существующая логика автоназначения.
5. Повторный вызов для того же плана и той же `next_due_date` не создаёт дубль, если уже есть открытая сгенерированная заявка.

Ограничение: у лифта должны быть координаты, потому что обычная заявка в текущем workflow требует `lat/lon`.

### Отметить выполнение
1. Во вкладке «ТО» нажать «Выполнено».
2. API `POST /api/maintenance-plans/<id>/complete` ставит `last_completed_date`.
3. Для `monthly`, `quarterly`, `semiannual` и `annual` следующая дата считается от даты выполнения, статус остаётся `active`.
4. Для `custom` без ручной `next_due_date` план получает статус `completed`.

Ограничения текущей основы:
- Нет cron-задач, автоуведомлений и автоматической генерации заявок по расписанию.
- Календарь сделан как практичная дата-группированная таблица, без drag-and-drop и тяжёлой frontend-библиотеки.
- Нет отдельного technician/PWA-потока для планов ТО.
- Если по плану нужна работа мастера в мобильном приложении, диспетчер создаёт обычную заявку из плана или вручную и дальше ведёт её существующим способом.

## Чистая локальная demo-база и безопасная очистка
Свежая локальная база после первого запуска содержит только штатный минимум: один admin, один dispatcher и пять активных мастеров/technician-пользователей (`master1` ... `master5`). Тестовые лифты/assets, заявки, архивные строки и случайные demo-объекты не создаются автоматически.

Перед любой ручной очисткой локальных данных выполните backup:
```bash
python scripts/backup_local.py
```

В этом PR нет автоматической destructive-миграции и нет скрипта, который удаляет существующие данные при старте. Если нужно полностью подготовить чистую локальную demo-базу, безопасный путь для разработки:
1. Остановить Flask-сервер.
2. Выполнить `python scripts/backup_local.py`.
3. Удалять или переносить `lift_crm.db`, `uploads/`, `archive.xlsx`, `archive_*.xlsx` и export-файлы только после backup и только если это локальная demo-среда, а не рабочая база.
4. Запустить `python app.py`; база создастся заново с минимальным штатным baseline.
